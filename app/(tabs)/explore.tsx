...
const CATEGORY_CARDS: {
  key: CategoryFilter; label: string; emoji: string; color: string; bg: string;
}[] = [
    { key: 'all', label: 'All Movies', emoji: '🍿', color: '#6C63FF', bg: '#1A1829' },
    { key: 'now_playing', label: 'Now Playing', emoji: '🎥', color: '#E63946', bg: '#2A1520' },
    { key: 'popular', label: 'Popular', emoji: '🔥', color: '#F5A623', bg: '#261D10' },
    { key: 'top_rated', label: 'Top Rated', emoji: '⭐', color: '#27AE60', bg: '#0E2018' },
    { key: 'upcoming', label: 'Upcoming', emoji: '📅', color: '#3498DB', bg: '#0F1D2A' },
  ];

function formatYear(date?: string) { return date?.slice(0, 4) ?? ''; }
  ...
  return (
    ...
    >
      ...
      <View style={mini.info}>
        <Text style={mini.title} numberOfLines={1}>{item.title}</Text>
...
const mini = StyleSheet.create({
  card: { width: 120, backgroundColor: '#1A1829', borderRadius: 12, overflow: 'hidden', marginRight: 10 },
      ...
      >
        ...
        <View style={styles.grid}>
          {CATEGORY_CARDS.map((cat) => {
            ...
            return (
              <TouchableOpacity
                key={cat.key}
                style={[styles.catCard, { backgroundColor: active ? cat.color : cat.bg, borderColor: active ? cat.color : '#252338' }]}
                activeOpacity={0.8}
                onPress={() => dispatch({ type: 'SET_CATEGORY', payload: cat.key })}
              >
                <Text style={styles.catEmoji}>{cat.emoji}</Text>
                <Text style={[styles.catLabel, { color: active ? '#FFFFFF' : cat.color }]}>{cat.label}</Text>
                <View style={[styles.catBadge, { backgroundColor: active ? 'rgba(255,255,255,0.2)' : cat.color + '25' }]}>
                  <Text style={[styles.catCount, { color: active ? '#FFFFFF' : cat.color }]}>{counts[cat.key]}</Text>
                </View>
              </TouchableOpacity>
            );
          })}
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>{activeCfg.emoji} {activeCfg.label}</Text>
            <Text style={styles.sectionCount}>{filteredMovies.length} films</Text>
          </View>

          {loading ? (
            <View style={styles.stateRow}><Text style={styles.stateText}>Loading movies…</Text></View>
          ) : filteredMovies.length === 0 ? (
            <View style={styles.stateRow}><Text style={styles.stateText}>No movies in this category</Text></View>
          ) : (
                ...
                <View style={styles.listInfo}>
                  ...
                  {item.contentRating ? (
                    <View style={styles.listRating}>
                      <Text style={styles.listRatingText}>{item.contentRating}</Text>
                    </View>
                  ) : null}
                </View>
...
const styles = StyleSheet.create({
  ...
  grid: { flexDirection: 'row', flexWrap: 'wrap', paddingHorizontal: 14, gap: 10, marginTop: 16, marginBottom: 8 },
  catCard: { width: '47%', borderRadius: 18, padding: 14, alignItems: 'flex-start', gap: 6, borderWidth: 1 },
  ...
  sectionTitle: { fontSize: 17, fontWeight: '800', color: '#FFFFFF', paddingHorizontal: 20, marginBottom: 12 },
  sectionCount: { fontSize: 12, color: '#555272' },

  listCard: {
    flexDirection: 'row', alignItems: 'center', backgroundColor: '#1A1829',
    marginHorizontal: 16, borderRadius: 14, marginBottom: 8, overflow: 'hidden',
  ...
  stateRow: { paddingHorizontal: 20, paddingVertical: 24, alignItems: 'center' },
